...
<!-- The example row's hash is the real digest of its own four fields against
     https://github.com/ebellumat/Polyphase-Engine, so it does not match this repository's URL - it is an
...
npx proof-of-usage hash \
  ...
  --repo "https://github.com/ebellumat/Polyphase-Engine"
```

```bash
printf '%s' "ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/ebellumat/Polyphase-Engine" | shasum -a 256
```

```python
import hashlib
hashlib.sha256("ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/ebellumat/Polyphase-Engine".encode()).hexdigest()
```
...
2. **There.** In the credits your product already publishes — credits screen, about page,
   ...
   ```
   Includes material from ebellumat/Polyphase-Engine — https://github.com/ebellumat/Polyphase-Engine
